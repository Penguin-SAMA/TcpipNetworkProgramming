...
<img src="./assets/image-20230323150511635.png" alt="image-20230323150511635" style="zoom:50%;" />
...
<img src="./assets/image-20230323151711718.png" alt="image-20230323151711718" style="zoom:50%;" />

套接字是以全双工方式工作的。也就是说，它可以双向传递数据。

对主机A首次传输的数据包的确认信息和为主机B传输数据做准备的同步信息捆绑发送，因此，此种类型的消息又称SYN+ACK。

### 5.2.3 TCP内部工作原理：与对方主机的数据交换

收发数据的默认方式如下图：

<img src="./assets/image-20230330151325346.png" alt="image-20230330151325346" style="zoom:50%;" />

若计时器发生超时则重传：

<img src="./assets/image-20230330151832986.png" alt="image-20230330151832986" style="zoom:50%;" />

### 5.2.4 TCP的内部工作原理：断开与套接字的连接

断开连接时，先由套接字A向套接字B传递断开连接的消息，套接字B发出确认收到的信息，然后向套接字A传递可以断开连接的消息，套接字A同样发出确认消息。

<img src="./assets/image-20230330152105934.png" alt="image-20230330152105934" style="zoom:50%;" />
...
只考虑可靠性，TCP比UDP好。但UDP在结构上比TCP更简洁。UDP不会发送类似ACK的应答消息，也不会像SEQ那样给数据包分配序号。因此，UDP的性能有时候比TCP高出很多。编程中实现UDP也比TCP简单。因此，在更重视性能而非可靠性的情况下，UDP是一种很好的选择。

为了提供可靠的数据传输服务，TCP在不可靠的IP层进行流控制，而UDP就缺少这种流控制机制。

### 6.1.2 UDP内部工作原理

<img src="./assets/image-20230330153430513.png" alt="image-20230330153430513" style="zoom:50%;" />

IP的作用就是让离开主机B的UDP数据包准确传递到主机A。但把UDP包最终交给主机A的某一UDP套接字的过程则是由UDP完成的。UDP最重要的作用就是根据端口号将传给主机的数据包交付给最终的UDP套接字。

### 6.1.3 UDP的高效使用

若要传递压缩文件，则必须使用TCP，因为压缩文件只要丢失一部分就很难解压。

对于多媒体数据而言，丢失一部分也没有太大问题，但因为需要提供实时服务，速度就成为非常重要的因素。
